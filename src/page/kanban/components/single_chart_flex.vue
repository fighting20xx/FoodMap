    ...
    <div class="chart-wrap">
        <div class="chart-title" :id="!isEdit ? 'beHidden' :''">
            <Select v-model="sourceId" style="width:100px">
                <Option v-for="item,index in dataSourceList" :value="item.sourceId" :key="item.sourceId">{{ item.name
                    }}
                </Option>
            </Select>
            ...
            <Button slot="append" @click="modifySubmit">执行</Button>
            </Input>
            <Button shape="circle" icon="md-close" @click="deleteSelf"></Button>
        </div>
        <div class="chart-main">
            ...
            <div class="chart-content" :id="chartId+'_height'" :class="isEdit ? 'padding-space' :''">
                ...
                <Table v-if="chartType ==='table'" class="chart-item" :columns="tableColume" :data="tableData"
	...
	export default  {
		...
		methods: {
			...
			getOption() {
				...
				this.$ajax(AJAX_VS_NODE).post('kanban/chart/getOption', {
			...
			executeSql() {
				...
				this.$ajax(AJAX_VS_NODE).post('kanban/chart/executeSql', {
					sql: this.chartSql,
					id: this.sourceId,
