...
((function util() {
	...
	var oUtil = window.util = {
		 ...
		 */
		formatSeconds: function(nSeconds, sTemplate = '{d} {h} {m} {s}', oUnit = {d: ' 天', h: ' 时', m: ' 分', s: ' 秒'}) {
			var t = nSeconds;
			var d, h, m, s;
		 ...
		 * @param {Array} aoData
		 * @param {String} sKey
		 * @param {Number|String|Object|……} oValue
		 * @return {Object}
		 */
		getArrayItemByKey: function (aoData, sKey, oValue) {
			for (let i = 0, l = aoData.length; i < l; i++) {
				let oItem = aoData[i];
				if (oItem[sKey] == oValue) {
					return oItem;
				}
			}
		 ...
		 * @param {Number|String|Object|……} oValue
		 */
		getSubArrayItemByKey: function (aaoData, sKey, oValue) {
			for (let i = 0, l = aaoData.length; i < l; i++) {
				let aoItem = aaoData[i];
				let oItem = this.getArrayItemByKey(aoItem, sKey, oValue);
		 ...
		 * @return {Array}
		 */
		getArrayItemsByKey: function (aoData, sKey, oValue) {
			let aoResult = [];
		 ...
		 * @return {Number}
		 */
		getArrayItemIndexByKey: function (aoData, sKey, oValue) {
			for (let i = 0, l = aoData.length; i < l; i++) {
				let oItem = aoData[i];
				if (oItem[sKey] == oValue) {
					return i;
		 ...
		 * @return {Array}
		 */
		getArrayItemValueByKey: function (aoData, sKey, oValue, sValueKey) {
			let oItem = this.getArrayItemByKey(aoData, sKey, oValue);
			if (oItem) {
				return oItem[sValueKey];
			}
			...
			if (typeStr === '[object Array]') {
				...
				for (var key in source) {
					if (source.hasOwnProperty(key)) {
						result[key] = oUtil.clone(source[key]);
					}
				}
			}
			return result;
